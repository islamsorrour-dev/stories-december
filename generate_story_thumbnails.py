# generate_story_thumbnails.py
# سكريبت لتوليد الصور المصغرة للقصص الـ 50

from PIL import Image, ImageDraw, ImageFont
import os

# قائمة القصص الـ 50
stories = [
    "آداب الطعام والشراب",
    "النظافة الشخصية",
    "احترام الوالدين",
    "آداب الحديث والاستئذان",
    "برّ الجيران",
    "الصدق والأمانة",
    "التعاون والمشاركة",
    "احترام المعلمين",
    "الالتزام في المدرسة",
    "النظام والترتيب",
    "مساعدة كبار السن",
    "الحفاظ على الممتلكات العامة",
    "آداب استخدام الهاتف",
    "آداب الطريق والمرور",
    "ضبط النفس وقت الغضب",
    "التواضع",
    "الاعتذار عند الخطأ",
    "حب الوطن",
    "احترام الآخر",
    "الصبر والاجتهاد",
    "الشكر والعرفان",
    "العطاء والعمل الجماعي",
    "احترام الوقت",
    "التفاعل الإيجابي في الأنشطة",
    "تقبل النقد",
    "الاهتمام بالبيئة",
    "تجنب التنمر",
    "الحفاظ على الخصوصية",
    "التعاون مع الأسرة",
    "التعبير عن المشاعر بشكل صحي",
    "مساعدة الزملاء",
    "احترام الاختلاف",
    "آداب استخدام الإنترنت",
    "ترتيب الأدوات بعد اللعب",
    "قول الحقيقة",
    "احترام الأدوار",
    "اختيار الأصدقاء",
    "الاعتناء بالحيوانات",
    "احترام القوانين",
    "المبادرة بالأعمال الطيبة",
    "قول \"من فضلك\" و\"شكرًا\"",
    "الحفاظ على الهدوء داخل الصف",
    "التحكم في الانفعالات",
    "التفكير قبل التصرف",
    "الالتزام بالواجبات",
    "مشاركة الألعاب",
    "احترام المعلمين",
    "حب التعلم",
    "الثقة بالنفس دون غرور",
    "تكوين عادات إيجابية يومية"
]

# الألوان المستخدمة في التصميم
colors = [
    ("#7b4397", "#dc2430"),  # بنفسجي - أحمر
    ("#dc2430", "#ffc107"),  # أحمر - أصفر
    ("#ffc107", "#7b4397"),  # أصفر - بنفسجي
    ("#7b4397", "#ffc107"),  # بنفسجي - أصفر
    ("#dc2430", "#7b4397"),  # أحمر - بنفسجي
]

# أيقونات مختلفة للقصص
icons = [
    "📚", "📖", "✨", "🌟", "💫",
    "🎯", "🎨", "🎭", "🎪", "🎬",
    "🏆", "🎁", "🎈", "🎉", "🎊",
    "💝", "💖", "💗", "💓", "💕",
    "🌺", "🌻", "🌷", "🌹", "🌸",
    "🌿", "🍀", "🌱", "🌳", "🌴",
    "⭐", "🌟", "✨", "💫", "⚡",
    "🔥", "💧", "🌊", "☀️", "🌙",
    "🌈", "☁️", "❄️", "🌍", "🌎",
    "🌏", "🗺️", "🏔️", "⛰️", "🌋"
]

def hex_to_rgb(hex_color):
    """تحويل لون hex إلى RGB"""
    hex_color = hex_color.lstrip('#')
    return tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))

def create_thumbnail(story_name, story_id, output_dir="images"):
    """إنشاء صورة مصغرة للقصة"""
    # إنشاء مجلد الإخراج إذا لم يكن موجوداً
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    # أبعاد الصورة
    width, height = 400, 300
    
    # إنشاء صورة بخلفية متدرجة
    color_index = (story_id - 1) % len(colors)
    color1, color2 = colors[color_index]
    
    rgb1 = hex_to_rgb(color1)
    rgb2 = hex_to_rgb(color2)
    
    # إنشاء صورة بخلفية متدرجة
    img = Image.new('RGB', (width, height), rgb1)
    draw = ImageDraw.Draw(img)
    
    # رسم متدرج بسيط
    for y in range(height):
        ratio = y / height
        r = int(rgb1[0] * (1 - ratio) + rgb2[0] * ratio)
        g = int(rgb1[1] * (1 - ratio) + rgb2[1] * ratio)
        b = int(rgb1[2] * (1 - ratio) + rgb2[2] * ratio)
        draw.line([(0, y), (width, y)], fill=(r, g, b))
    
    # محاولة استخدام خط عربي (إذا كان متوفراً)
    font_large = None
    font_small = None
    
    # قائمة مسارات الخطوط المحتملة
    font_paths = [
        "C:/Windows/Fonts/arial.ttf",
        "C:/Windows/Fonts/tahoma.ttf",
        "C:/Windows/Fonts/segoeui.ttf",
        "arial.ttf",
        "tahoma.ttf"
    ]
    
    for font_path in font_paths:
        try:
            if os.path.exists(font_path):
                font_large = ImageFont.truetype(font_path, 40)
                font_small = ImageFont.truetype(font_path, 20)
                break
        except:
            continue
    
    if font_large is None:
        # استخدام خط افتراضي
        font_large = ImageFont.load_default()
        font_small = ImageFont.load_default()
    
    # إضافة نص اسم القصة
    text = story_name
    
    # حساب موضع النص (في المنتصف)
    # استخدام textbbox للحصول على أبعاد النص
    try:
        bbox = draw.textbbox((0, 0), text, font=font_large)
        text_width = bbox[2] - bbox[0]
        text_height = bbox[3] - bbox[1]
    except:
        # في حالة الخط الافتراضي
        text_width = len(text) * 20
        text_height = 40
    
    x = (width - text_width) // 2
    y = (height - text_height) // 2 - 10
    
    # رسم خلفية شبه شفافة للنص (بيضاء مع شفافية)
    padding = 20
    overlay = Image.new('RGBA', (width, height), (0, 0, 0, 0))
    overlay_draw = ImageDraw.Draw(overlay)
    overlay_draw.rectangle(
        [x - padding, y - padding, x + text_width + padding, y + text_height + padding],
        fill=(255, 255, 255, 220)
    )
    img = Image.alpha_composite(img.convert('RGBA'), overlay).convert('RGB')
    draw = ImageDraw.Draw(img)
    
    # رسم النص باللون الداكن
    draw.text((x, y), text, fill=(50, 50, 50), font=font_large)
    
    # إضافة رقم القصة في الزاوية
    story_num = f"#{story_id}"
    bbox_num = draw.textbbox((0, 0), story_num, font=font_small)
    num_width = bbox_num[2] - bbox_num[0]
    num_height = bbox_num[3] - bbox_num[1]
    
    # خلفية للرقم
    draw.ellipse(
        [width - 50, 10, width - 10, 50],
        fill=(255, 255, 255, 230)
    )
    draw.text((width - 50 + (40 - num_width) // 2, 10 + (40 - num_height) // 2), 
              story_num, fill=(123, 67, 151), font=font_small)
    
    # حفظ الصورة
    output_path = os.path.join(output_dir, f"story-{story_id}.jpg")
    img.save(output_path, "JPEG", quality=95)
    print(f"✅ تم إنشاء: {output_path} - {story_name}")

def main():
    print("🚀 بدء توليد الصور المصغرة للقصص...\n")
    
    for i, story_name in enumerate(stories, 1):
        try:
            create_thumbnail(story_name, i)
        except Exception as e:
            print(f"❌ خطأ في إنشاء صورة القصة {i}: {e}")
    
    print(f"\n✨ تم إنشاء {len(stories)} صورة مصغرة بنجاح!")

if __name__ == "__main__":
    main()

